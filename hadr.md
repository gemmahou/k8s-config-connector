...
1. **Prepare test clusters**
   ...
   3. Create test clusters
      ...
      ```
      To create a private cluster with workload identity enabled:
      ```bash
      gcloud beta container clusters create "${cluster_name}" \
                    ...
                    --master-ipv4-cidr="${cidr}"
      # `cidr` has to be different if you are using the same VPC network
      
      gcloud container clusters get-credentials ${cluster_name} --location=${location} --dns-endpoint
      ```
...
2. **KCC Installation:**
   ...
   2. Configure the operator as cluster mode
      ...
      ```
   2. Wait and Verify your installation 
      ```bash
